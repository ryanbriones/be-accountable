...
module BeAccountable
  # From topfunky's calendar_helper
  ...
  module CalendarHelper
    ...
    def calendar(options = {}, &block)
      ...
      beginning_of_week(first, first_weekday).upto(beginning_of_week(last, first_weekday) + 6) do |cur|
        cell_text, cell_attrs = block.call(cur)
        cell_text  ||= cur.mday
        cell_attrs ||= {}
        cell_attrs[:class] ||= options[:day_class]
        cell_attrs[:class] += " #{options[:other_month_class]}" if first.month != cur.month
        ...
        if (first.month != cur.month) && options[:accessible]
          cal << "<span class='hidden'> #{Date::MONTHNAMES[cur.mon]}</span>"
        end
        cal << "</td>"
